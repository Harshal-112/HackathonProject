...
//    (Independent of XAI algorithm / decision features)
// ---------------------------------------------------------------------------
export function detectPII(text) {
  ...
  while ((m = aadhaarRe.exec(text)) !== null) {
    ...
    results.push({
      type: 'Aadhaar',
      original: m[0],
      masked: `XXXX XXXX ${digits.slice(-4)}`,
      icon: 'Fingerprint',
      severity: 'high',
  ...
  while ((m = panRe.exec(text)) !== null) {
    results.push({
      type: 'PAN Card',
      original: m[0],
      masked: `${m[0].slice(0, 3)}XX${m[0].slice(5, 9)}X`,
      icon: 'CreditCard',
      severity: 'high',
  ...
  // GSTIN: 15-char Indian GST format
  const gstRe = /\b\d{2}[A-Z]{5}\d{4}[A-Z]{1}[A-Z\d]{1}[Z]{1}[A-Z\d]{1}\b/g
  while ((m = gstRe.exec(text)) !== null) {
    results.push({
      type: 'GSTIN',
      original: m[0],
      masked: `${m[0].slice(0, 2)}XXXXX${m[0].slice(7)}`,
      icon: 'Building2',
      severity: 'medium',
  ...
  while ((m = phoneRe.exec(text)) !== null) {
    results.push({
      type: 'Phone Number',
      original: m[0],
      masked: `${m[0].slice(0, Math.max(m[0].length - 4, 3))}XXXX`,
      icon: 'Phone',
      severity: 'medium',
